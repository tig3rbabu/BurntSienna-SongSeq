...
in_thread do
  live_loop :bass do
    with_fx :gverb, room: 60, release: 4, mix: 0.2 do
      tick
...
in_thread do
  live_loop :synth do
    with_fx :gverb, room: 70, release: 8, mix: 0.2, amp: 1 do
      with_fx :echo, phase: 0.5*1, decay: 4, mix: 0.7 do
        with_fx :slicer, phase: 0.5*2, wave: 1, pulse_width: 0.25, mix: 1 do
...
in_thread do
  live_loop :synth34 do
    with_fx :gverb, room: 70, release: 4, mix: 0.4, amp: 1.2 do
  ...
  live_loop :beat4 do
    ...
    with_fx :level, amp: 1.5 do
      with_fx :echo, phase: 0.5*2, decay: 4, mix: 0.5 do
        ...
        with_fx :gverb, room: 50, tail_level: 0.9, release: 3, mix: 0.8, damp: 0 do
          ##| sample :elec_pop, amp: 3, cutoff: 110, rate: 2.2
